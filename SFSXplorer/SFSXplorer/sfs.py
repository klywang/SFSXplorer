...
class Explorer(object):
    ...
    def __init__(self,sfs_input):
        """Constructor method"""
        
        # Set up attribute
        self.sfs_input = sfs_input
    ...
    def read_input(self):
        ...
        for line in csv:
            if line[0] == "#":
                ...
                self.l_i =  handle_hash("float",line[1])                    
            elif line[0] == "l_f":
                self.l_f =  handle_hash("float",line[1])  
            elif line[0] == "n_l":
                self.n_l = handle_hash("int",line[1])
                ...
                self.e0_i = handle_hash("float",line[1])
            elif line[0] == "e0_f":
                self.e0_f = handle_hash("float",line[1])
            elif line[0] == "n_e0":
                self.n_e0 = handle_hash("int",line[1])
            
            elif line[0] == "m_sol_i":
                self.m_sol_i = handle_hash("int",line[1])
            elif line[0] == "m_sol_f":
                self.m_sol_f = handle_hash("int",line[1])
            elif line[0] == "n_m_sol":
                self.n_m_sol = handle_hash("int",line[1])
            
            elif line[0] == "n_sol_i":
                self.n_sol_i = handle_hash("int",line[1])
            elif line[0] == "n_sol_f":
                self.n_sol_f = handle_hash("int",line[1])
            elif line[0] == "n_n_sol":
                self.n_n_sol = handle_hash("int",line[1])
                
            elif line[0] == "sigma_sol_i":
                self.sigma_sol_i = handle_hash("float",line[1])
            elif line[0] == "sigma_sol_f":
                self.sigma_sol_f = handle_hash("float",line[1])
            elif line[0] == "n_sigma_sol":
                self.n_sigma_sol = handle_hash("int",line[1])
        
        # Check whether all parameters have been read
        print("\nParameters read from sfs.in file:")
        try:
            print("l_i,l_f,n_l:",self.l_i,self.l_f,self.n_l)
            print("k_i,k_f,n_k:",self.k_i,self.k_f,self.n_k)
            print("a_i,a_f,n_a:",self.a_i,self.a_f,self.n_a)
            print("e0_i,e0_f,n_e0:",self.e0_i,self.e0_f,self.n_e0)
            print("m_sol_i,m_sol_f,n_m_sol:",self.m_sol_i,self.m_sol_f,self.n_m_sol)
            print("n_sol_i,n_sol_f,n_n_sol:",self.n_sol_i,self.n_sol_f,self.n_n_sol)
            print("sigma_sol_i,sigma_sol_f,n_sigma_sol:",self.sigma_sol_i,self.sigma_sol_f,self.n_sigma_sol)
        except:
            sys.exit("\nError! Missing input parameters. Please check sfs.in file!")
    ...
    def write_energy(self):
        ...
        e0_array = np.linspace(self.e0_i,self.e0_f,self.n_e0)
        k_array = np.linspace(self.k_i,self.k_f,self.n_k)
        l_array = np.linspace(self.l_i,self.l_f,self.n_l)
            ...
            for e0 in e0_array:
                for k in k_array:
                    for l in l_array:
                        ...
                        v_Elec_logistic_headers += "v_Elec_Log_"+str(a)+"_"+str(e0)+"_"+str(k)+"_"+str(l)+","
                        v_Elec_tanh_headers += "v_Elec_Tanh_"+str(a)+"_"+str(e0)+"_"+str(k)+"_"+str(l)+","
                        v_Elec_logistic_tanh_headers += "v_Elec_Log_Tanh_"+str(a)+"_"+str(e0)+"_"+str(k)+"_"+str(l)+","
        ...
        # Set up arrays
        m_array_sol = np.linspace(self.m_sol_i,self.m_sol_f,self.n_m_sol)
        n_array_sol = np.linspace(self.n_sol_i,self.n_sol_f,self.n_n_sol)
        s_array_sol = np.linspace(self.sigma_sol_i,self.sigma_sol_f,self.n_sigma_sol)
                
        # Looping through m_array_sol, n_array_sol, and s_array_sol
        for m in m_array_sol:
            for n in n_array_sol:
                for sigma in s_array_sol:
            
                    # Set up headers for solvatation potential
                    v_Sol_headers += "v_Sol_"+str(m)+"_"+str(n)+"_"+str(sigma)+","
        ...
        v_Elec_tanh_headers+v_Elec_logistic_tanh_headers+v_Sol_headers+"Torsions,Experimental\n")
        ...
        for line in self.csv0:
            ...
            if line[0] == "CHKLIG":
                ...
                for m in m_array_sol:
                    for n in n_array_sol:
                        for sigma in s_array_sol:
                            ...
                            # Invoking intermol_pot_Sol() method
                            v_Sol_pot = pot.intermol_pot_Sol(par_list,lig_list,receptor_list,m,n,sigma)
                            v_Sol += str(v_Sol_pot)+","
                ...
                aux_line_LJ_HB+\
                ","+v_Elec_logistic+v_Elec_tanh+v_Elec_logistic_tanh+v_Sol+str(number_torsions)+","+str(exp_aff)+"\n")
