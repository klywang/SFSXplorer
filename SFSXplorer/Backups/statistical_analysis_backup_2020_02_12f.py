...
class StatsAnalysis(object):
    ...
    def __init__(self,file_in,file_out,dir_in,dir_out,exp_string):
        """Constructor method"""
    ...
    def process_it(self):
        ...
        for i in range(n_cols):
            ...
            # Invoke additional_metrics(Y)
            mse_array[i],rmse_array[i] = st1.additional_metrics(x,Y)
        
        # Show data results (write first line)
        fo1.write("Energy Term,rho,p-value1,r,p-value2,r2,MSE,RMSE\n")
        ...
        for i in range(n_cols-1):
            if i < n_cols-1:
                ...
                ","+str(mse_array[i])+","+str(rmse_array[i])+"\n")
            else:
                fo1.write(self.headers[i+1]+","+str( rho_array[i])+","+str(p_v1_array[i])+\
                ","+str(r_array[i])+","+str(p_v2_array[i])+","+str(r2_array[i])+\
                +","+str(mse_array[i])+","+str(rmse_array[i]))
        
        #self.sort_it(rho_array,self.index_experimental)
